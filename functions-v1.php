...
/* Add shortcode for Parent Page Title 
function ihack_parent_title() {
	global $post;
  	if ( $post->post_parent ) {
	    $parentlink = '<a href="' . echo get_permalink( $post->post_parent ); .'" >'. get_the_title($post->post_parent ); .' </a>';
}
	echo $parentlink;
}
add_shortcode('ihack_parent', 'ihack_parent_title');*/
...
function wps_parent_post(){
  ...
  if($post->ID != $parent){
      echo '<div class="ihackparent"><a href="'.get_permalink($parent).'" class="parent-post">Back to parent page</a></div>';
  }
}
...
echo 'Welcome to iHackOnline by Disruptors Co - the easiest way to meet and collaborate online. For support email gavin@disruptorsco.com.';
}
